...
const MainLayout = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans transition-colors ${isHome ? 'bg-navy text-white' : 'bg-navy text-slate/60'}`}>
      <header className={`w-full ${isHome ? 'absolute top-0 z-50' : 'sticky top-0 z-40 bg-navy/70 border-b border-charcoal/80 backdrop-blur-xl'}`}>
        <div className={`w-full max-w-[1800px] mx-auto px-5 sm:px-8 md:px-[82px] ${isHome ? 'pt-[20px] sm:pt-[30px]' : 'h-20'} flex items-center justify-between gap-4`}>
          <NavLink to="/" className="flex items-center gap-2.5 shrink-0" onClick={() => setIsOpen(false)}><div className="w-7 h-7 sm:w-8 sm:h-8 rounded-[9px] bg-white text-navy flex items-center justify-center font-[450] text-lg">G</div><div><h1 className="text-white text-[22px] sm:text-[26px] font-[450] leading-none tracking-[-0.02em]">ShopGenie AI</h1><p className="text-[10px] text-white/55 font-medium mt-1">Smart Retail Assistant</p></div></NavLink>
          ...
          <div className="hidden lg:flex h-[52px] p-[3px] bg-[rgba(0,0,0,0.35)] rounded-[13px] backdrop-blur-[17px] items-center gap-[5px]">
            ...
            <NavLink to="/customer/login" className="h-[46px] px-5 rounded-[11px] text-white text-[14px] font-[450] flex items-center gap-2 hover:bg-white/5 transition-colors"><LogIn className="w-4 h-4" />User Login</NavLink>
            <NavLink to="/shop-owner/login" className="h-[46px] px-5 bg-[#E9E9E9] rounded-[11px] text-navy text-[14px] font-[450] flex items-center gap-2 hover:bg-white transition-colors"><Store className="w-4 h-4" />For shop owners</NavLink>
          </div>
