...
const MainLayout = () => {
  ...
  const isLightShell = isHome || pathname === '/login';

  return (
    <div className={`min-h-screen flex flex-col font-sans transition-colors ${isLightShell ? 'bg-sky-50 text-slate-900' : 'bg-slate-950 text-slate-100'}`}>
      {/* Top Header */}
      <header className={`h-20 border-b px-4 sm:px-8 flex items-center justify-between gap-4 sticky top-0 z-40 backdrop-blur-xl ${isLightShell ? 'border-sky-100 bg-white/90 shadow-sm' : 'border-slate-800/80 bg-slate-950/70'}`}>
        <NavLink to="/" className="flex items-center gap-3 shrink-0">
          <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center text-white font-bold text-2xl">
            G
          </div>
          <div>
            <h1 className="font-normal text-base tracking-tight text-black">ShopGenie AI</h1>
            <p className="text-[10px] text-slate-400 font-medium">Smart Retail Assistant</p>
          </div>
        </NavLink>

        <nav className={`hidden lg:flex items-center gap-8 text-sm font-medium ${isLightShell ? 'text-slate-600' : 'text-slate-300'}`}>
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-3 shrink-0">
          <button
            ...
            className={`p-2.5 rounded-2xl border transition-all shrink-0 ${isLightShell ? 'bg-white border-sky-200 text-slate-600 hover:text-sky-700' : 'bg-slate-900 border-slate-800 text-slate-300 hover:text-white'}`}
          >
          ...
          <NavLink
            ...
            title="Sign in"
            aria-label="Sign in"
            className={`px-3 sm:px-4 py-2.5 rounded-xl border text-xs sm:text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap ${isLightShell ? 'border-sky-200 text-slate-700 bg-white hover:bg-sky-50' : 'border-white text-white bg-slate-900/40 hover:bg-slate-800'}`}
          >
          ...
          <NavLink to="/register?role=customer" className={`hidden sm:flex items-center gap-2 px-3 py-2.5 rounded-xl border text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${isHome ? 'border-sky-200 text-slate-700 hover:border-cyan-400 hover:text-slate-900' : 'border-slate-700 text-slate-200 hover:border-cyan-400 hover:text-white'}`}>
            <ShoppingBag className="w-4 h-4 -translate-y-0.5 text-cyan-400" />
