      ...
      <div className="relative w-full max-w-[1800px] mx-auto px-5 sm:px-8 md:px-[82px] pt-28 sm:pt-32 pb-12">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-10 lg:gap-12">
          <div className="max-w-[593px]">
            ...
            <Animate delay={700}><NavLink to="/login?role=customer" className="inline-flex h-[46px] sm:h-[51px] items-center gap-3 px-5 sm:px-[27px] bg-[#E9E9E9] rounded-[12px] text-[#0A0707] text-[14px] sm:text-[15.5px] font-[450] transition-opacity hover:opacity-90">Find products near you <ArrowRight className="w-4 h-4" /></NavLink></Animate>
          </div>
          <Animate delay={900} direction="scale" className="w-full max-w-[405px] mx-auto lg:mx-0"><div className="rounded-[24px] sm:rounded-[33px] bg-[rgba(17,16,15,0.45)] backdrop-blur-[20px] border border-white/[0.08] p-5 sm:p-8 pb-5 sm:pb-6"><p className="text-white text-[16px] sm:text-[20px] font-[450] leading-[20px]">Choose your experience</p><h2 className="text-white/65 text-sm mt-2 mb-5 sm:mb-7">One platform. Two simple ways in.</h2><div className="space-y-3"><ExperienceCard to="/login?role=customer" icon={ShoppingBag} title="I’m a customer" description="Explore products, discover nearby shops, check prices and availability, and stay connected with the shops you trust." action="User Login" /><ExperienceCard to="/register?role=owner" icon={Store} title="I run a shop" description="Manage products, inventory, billing, alerts, and sales performance from one focused workspace." action="Set up my shop" /></div></div></Animate>
        </div>
      </div>
    </section>
