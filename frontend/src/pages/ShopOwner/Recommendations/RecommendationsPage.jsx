...
const RecommendationsPage = () => {
  ...
  return (
    <div className="space-y-12 max-w-6xl mx-auto">
      {/* Header */}
      <div>
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-charcoal/10 border border-charcoal/30 text-purple-400 text-xs font-semibold mb-2">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold text-slate/60 flex items-center gap-2">
            <ThumbsUp className="w-5 h-5 text-indigo-400" /> Frequently Bought Together
          </h2>
          <span className="text-xs text-slate/80">High Co-Occurrence Pairings</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {(data.frequently_bought_together || []).map((p, idx) => (
            <GlassCard key={idx} className="bg-charcoal/60 border-charcoal space-y-3">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs text-slate/80">Content-Based Vector Matching</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {(data.similar_products || []).map((p, idx) => (
            <GlassCard key={idx} className="bg-charcoal/60 border-charcoal space-y-3">
              ...
              <button
                onClick={() => addToast(`Added ${p.name} to POS Cart!`, 'success')}
                className="w-full py-2.5 rounded-xl bg-charcoal hover:bg-charcoal text-white text-xs font-bold flex items-center justify-center gap-2"
              >
                <ShoppingCart className="w-4 h-4" /> Add to Sale
              </button>
            </GlassCard>
