...
const FilterDrawer = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-navy/80 backdrop-blur-sm animate-fade-left">
      <div className="relative w-full max-w-md bg-charcoal border-l border-white/10 p-6 shadow-2xl text-white flex flex-col justify-between h-full overflow-hidden">
        <div className="min-h-0 flex-1 overflow-hidden">
          ...
          <div className="mt-6 space-y-6">
            ...
            {/* Category Filter */}
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-slate/80 block mb-3">
                Category
              </label>
                ...
                <button
                  ...
                  className={`rounded-xl px-3 py-1.5 text-xs font-semibold transition-all ${
                    selectedCategory === 'all'
                      ...
                      : 'bg-white/[0.05] border border-white/10 text-slate/60 hover:bg-white/10'
                  }`}
                >
                  All Categories
                </button>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold uppercase tracking-wider text-slate/80">
                  Max Price Range
                </label>
              ...
              <input
                ...
                className="w-full h-2 bg-charcoal rounded-lg appearance-none cursor-pointer accent-cyan-400"
              />
            ...
            <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.04] p-3.5">
              <div>
                ...
                <p className="text-[10px] text-slate/80">Hide products currently out of stock</p>
              </div>
              <button
                onClick={() => setOnlyInStock(!onlyInStock)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  onlyInStock ? 'bg-amber' : 'bg-charcoal'
                }`}
              >
                <span
            ...
            {setMinRating && (
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-slate/80 block mb-2">
                  ...
                  {[0, 4.0, 4.5, 4.8].map((rating) => (
                    <button
                      ...
                      className={`flex-1 rounded-xl py-2 text-xs font-semibold border transition-all flex items-center justify-center gap-1 ${
                        minRating === rating
                          ? 'border-cyan-400 bg-amber/20 text-cyan-300'
                          : 'border-white/10 bg-white/[0.05] text-slate/80 hover:bg-white/10'
                      }`}
                    >
                      {rating === 0 ? (
                        'All'
                  ...
                  <div className="space-y-1.5 p-2">
                    <button
                      ...
                      className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-xs transition-colors ${
                        selectedShop === 'all'
                          ? 'bg-amber/20 text-cyan-300 ring-1 ring-amber/30'
                          : 'text-slate/60 hover:bg-white/5'
                      }`}
                    >
        ...
        <div className="pt-6 border-t border-white/10 flex items-center gap-3">
          <button
            onClick={onReset}
            className="flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.05] py-3 px-4 text-xs font-semibold text-slate/60 hover:bg-white/10 transition-all flex-1"
          >
