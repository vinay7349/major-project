...
const CustomerLoginPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#eef3fb] via-[#f6f7fc] to-[#eaeef9] text-navy font-sans relative overflow-hidden flex flex-col">
      {/* Ambient light blobs */}
      ...
      {/* Top brand bar */}
      <header className="relative z-20 w-full border-b border-charcoal/5 bg-white/60 backdrop-blur-xl">
          ...
          <NavLink to="/" className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xl font-bold tracking-tight text-navy">ShopGenie</span>
              ...
              <p className="text-[11px] text-slate">Find products &amp; local shops near you</p>
            </div>
          </NavLink>
        </div>
      </header>
          ...
          <div className="lg:col-span-7 relative">
            <div className="hidden lg:block relative h-[560px]">
              <div className="absolute left-[6%] top-[6%] h-36 w-36 rounded-3xl bg-white shadow-[0_25px_60px_-15px_rgba(79,70,229,0.35)] border border-white p-3 rotate-[-7deg]">
                <div className="h-full rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center"><Package className="h-12 w-12 text-charcoal" /></div>
            ...
            <div className="lg:hidden text-center max-w-md mx-auto mb-8">
              <div className="inline-flex items-center justify-center h-16 w-16 rounded-2xl bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-600 shadow-xl shadow-charcoal/30 mb-4"><ShoppingBag className="h-8 w-8 text-white" /></div>
              <h1 className="text-3xl font-extrabold tracking-tight text-navy">Welcome back</h1>
              <p className="mt-2 text-slate text-sm">Access your saved items &amp; customer profile.</p>
            </div>
          </div>
            ...
            <div className="rounded-[28px] border border-white/70 bg-white/80 backdrop-blur-2xl p-6 sm:p-9 shadow-[0_40px_90px_-30px_rgba(79,70,229,0.35)] space-y-6 relative overflow-hidden">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-lg shadow-charcoal/25"><ShoppingBag className="h-5 w-5" /></div>
                  <div>
                    <h2 className="text-xl font-extrabold tracking-tight text-navy">Customer Sign In</h2>
                    <p className="text-xs text-slate">Access your saved items &amp; customer profile</p>
                  </div>
                </div>
              </div>
              <button onClick={handleDemoCustomerLogin} type="button" className="w-full flex items-center justify-center gap-2.5 rounded-xl border border-charcoal/30 bg-gradient-to-r from-blue-600/10 via-indigo-600/10 to-violet-600/10 py-3 px-4 text-xs font-bold text-charcoal hover:border-charcoal/60 hover:from-blue-600/15 hover:to-violet-600/15 hover:text-charcoal transition-all shadow-sm">
                <Zap className="h-4 w-4 text-charcoal fill-indigo-500" />
                <span>Continue with 1-Click Demo Customer</span>
              </button>
              <div className="relative flex items-center justify-center">
                <div className="w-full border-t border-warmwhite/60" /><span className="absolute bg-white px-3 text-[11px] uppercase font-bold tracking-wider text-slate/80">Or</span>
              </div>
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-charcoal mb-1.5">Email Address</label>
                  <div className="relative">
                    <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate/80" />
                    <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="customer@example.com" className="h-12 w-full rounded-xl border border-warmwhite/60 bg-white pl-10 pr-4 text-xs sm:text-sm text-navy placeholder-slate-400 outline-none focus:border-charcoal/60 focus:ring-2 focus:ring-charcoal/15 transition-all shadow-sm" />
                  </div>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-xs font-semibold text-charcoal">Password</label>
                    <NavLink to="/forgot-password" className="text-xs font-medium text-charcoal hover:text-charcoal transition-colors">Forgot password?</NavLink>
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate/80" />
                    <input type={showPassword ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="h-12 w-full rounded-xl border border-warmwhite/60 bg-white pl-10 pr-10 text-xs sm:text-sm text-navy placeholder-slate-400 outline-none focus:border-charcoal/60 focus:ring-2 focus:ring-charcoal/15 transition-all shadow-sm" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate/80 hover:text-slate transition-colors">
                      {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
                <button type="submit" disabled={loading} className="w-full h-12 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs sm:text-sm font-bold hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg shadow-charcoal/25 hover:shadow-xl hover:shadow-charcoal/30 disabled:opacity-50">
                  {loading ? 'Signing In...' : 'Sign In as Customer'}
                </button>
              </form>
              <div className="pt-4 border-t border-slate-100 text-center">
                <p className="text-xs text-slate">New to ShopGenie?{' '}<NavLink to="/register?role=customer" className="font-bold text-charcoal hover:text-charcoal transition-colors">Create Customer Account</NavLink></p>
              </div>
            </div>
          </div>
        </div>
