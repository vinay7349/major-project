...
const ShopDetailModal = ({ shop, products = [], onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-navy/80 backdrop-blur-md animate-fade-scale">
      <div className="relative w-full max-w-2xl rounded-3xl border border-white/10 bg-charcoal/95 p-6 sm:p-8 shadow-2xl text-white my-8 max-h-[90vh] overflow-y-auto">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex items-start gap-4 pb-6 border-b border-white/10">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="rounded-full bg-amber/10 px-2.5 py-0.5 text-xs font-semibold text-cyan-300 border border-amber/20">
                {shop.category || 'General Store'}
              </span>
              {shop.is_open !== false ? (
                <span className="rounded-full bg-mutedgreen/10 px-2.5 py-0.5 text-xs font-semibold text-emerald-400 border border-mutedgreen/20 flex items-center gap-1">
                  <Clock className="h-3 w-3" /> Open Now
                </span>
              ) : (
                <span className="rounded-full bg-red/10 px-2.5 py-0.5 text-xs font-semibold text-rose-400 border border-red/20">
                  Closed
                </span>
              )}
            </div>

            <div className="mt-2 flex items-center gap-4 text-xs text-slate/60 flex-wrap">
              <span className="flex items-center gap-1 text-amber-400 font-semibold">
                <Star className="h-3.5 w-3.5 fill-amber-400" /> {shop.rating || '4.8'} ⭐
              </span>
              {shop.distance_km && (
                <span className="flex items-center gap-1 text-slate/60">
                  <MapPin className="h-3.5 w-3.5 text-cyan-400" /> {shop.distance_km} km away
                </span>
              )}
              {shop.owner_name && (
                <span className="flex items-center gap-1 text-slate/80">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-xs">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <p className="font-semibold text-slate/60">Location & Address</p>
              <p className="mt-0.5 text-slate/80 leading-normal">{shop.address || 'Market Street, Sector 4'}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <p className="font-semibold text-slate/60">Contact Number</p>
              <p className="mt-0.5 text-slate/80 font-mono">{shop.phone || '+1 (555) 234-5678'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate/80">{shopProducts.length} items</span>
          </div>
            ...
            {shopProducts.slice(0, 6).map((item) => (
              <div key={item.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] p-3">
                <img
                  ...
                  alt={item.name}
                  className="h-12 w-12 rounded-lg object-cover shrink-0 bg-navy border border-white/10"
                />
